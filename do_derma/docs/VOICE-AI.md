...
| After **Stop** | Transcript → AI note. The draft is saved on the encounter immediately (still a draft) and the panel opens for editing. Diagnosis + ICD‑10 chip, collapsible Arabic note and WhatsApp follow-up (EN/AR, Copy). |
| Format toggle | **Structured / SOAP / H&P**. Dictation fills SOAP or H&P (Structured switches to SOAP). H&P = Chief Complaint, History of Presenting Complaint, Past Medical History, Examination Findings, Assessment, Management Plan. |
...
1. `bench get-app` both apps on the branch, `bench --site <site> migrate` (creates custom fields, the `Derma AI Usage` doctype and the four `Derma AI *` print templates), `bench build --app do_derma`.
...
3. Make sure each Healthcare Practitioner has `practitioner_name` and `designation` (used in sign-offs) and the Company has `company_name` and `country` (used in prompts).
4. Server needs `wkhtmltopdf` (standard Frappe) for Issue → PDF.
5. Workers: with `bench start`/production the AI runs on the `long` queue and the page polls; with a bare `bench serve` (no workers) it runs inline.
...
| `do_derma.voice.generate_note` | same as queue_note (synchronous) | `{encounter, values{SOAP fields}, hp_values{H&P fields}, diagnosis, icd10, soap_ar, followup_en, followup_ar}` |
...
`bench --site <test site> run-tests --app do_derma --module do_derma.tests.test_voice` (and `test_hp_mode`, `test_documents`, `test_ai_ops`). External calls are mocked; PDF rendering uses a blank PDF.
